...
# Fix host-mounted repo trust + stale OCaml generator binaries.
# - Git may block mounted repos with "dubious ownership".
# - *.out generators built with a different OCaml version fail with magic mismatch.
if [ -d /home/pprz/paparazzi/.git ]; then
    gosu "$USER_NAME" git config --global --add safe.directory /home/pprz/paparazzi || true
fi

if [ -d /home/pprz/paparazzi/sw/tools/generators ]; then
    find /home/pprz/paparazzi/sw/tools/generators -maxdepth 1 -type f -name '*.out' -delete 2>/dev/null || true
fi
